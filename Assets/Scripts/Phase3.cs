...
public class Phase3 : MonoBehaviour
{
    ...
    [SerializeField] private int _strAmountInt;
    ...
    {
        _inputTroops = str;
        Debug.Log("input value is " + _inputTroops);
        int.TryParse(_inputTroops, out int _outputInt);
        _strAmountInt = _outputInt;
        HideInputText();
        switch (buttonName)
        {
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("PHase3:: Player Actions is null");
        }
        //
        _uiManager = GameObject.Find("UI").GetComponent<UIManager>();
        if (_uiManager == null)
        {
            Debug.LogError("PHase3:: UIManager is null");
        }
    }
    ...
    {
        StartCoroutine(MakeBricksCo(_strAmountInt));
    }
    ...
    IEnumerator BuildWallsCo(int amount)
    {
        ...
        if (_playerActions.idleTroopCount != 0)
        {
            ...
            _uiManager.DisplayTotalWallsPhase2(_miles);
    ...
    {
        StartCoroutine(BuildWallsCo(_strAmountInt));
    }
    ...
    IEnumerator DigTrenchesCo(int amount)
    {
        ...
        if (_playerActions.idleTroopCount != 0)
        {
            ...
            _playerActions.AddIdleTroops(amount);
            _uiManager.DisplayTotalTrenchesPhase2(_currentTrenches);
        }
        else
        {
            _playerActions.StartNOMore();
    ...
    {
        StartCoroutine(DigTrenchesCo(_strAmountInt));
    }
    ...
    {
        StartCoroutine(ChopWoodCo(_strAmountInt));
    }
    ...
    IEnumerator BuildTowers(int amount)
    {
        ...
        if (_playerActions.idleTroopCount != 0)
        {
            ...
            _uiManager.DisplayTotalTowersPhase2(_currentTowers); //display towers number
    ...
    {
        StartCoroutine(BuildTowers(_strAmountInt));
    }
